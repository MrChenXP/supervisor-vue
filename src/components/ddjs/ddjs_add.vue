<template>
  <div class="c_ddjs_add c_ContentMain">
    <!--标题栏 $router.push({path: '/ebc60e699bc642a1871f1e017b979483'})-->
    <x-header class="c_header" :left-options="{preventGoBack: true}" @on-click-back="toBack">
      <span v-html="funName"></span>
    ...
    <group class="c_FormAdd" v-show="!ddpgShow">
      <cell title="工作计划" :value="workPlanName" v-if="contentId"></cell>
      <cell title="工作计划" is-link @click.native="showWrokPlan" :value="workPlanName" v-else></cell>
      <cell title="学校(园)" :value="data.xxName" v-if="contentId"></cell>
      <cell title="学校(园)" is-link  @click.native="toXx" :value="data.xxName" v-else></cell>
      <!--<cell title="随行督学" is-link @click.native="showSxdx" :value="sxdxName"></cell>-->
      <cell title="随行督学" is-link @click.native="showSxdx" :value="sxdxName"></cell>
      <datetime title="时间" :end-date="endDate" :start-date="startDate" v-model="data.YWSJ"></datetime>
      <!--督导事项-->
      <div class="c_FormAdd_editor" v-if="!!workPlanId">
        ...
        <div v-show="ddsxShow" class="c_FormAdd_editor_content">
          ...
          <div class="c_upLoadImg_img">
            <div v-for="(item, index) in previewerListDDSX" :key="index" class="c_ddjs_zgtz_xzbody_ddjsbody_imgBody">
      ...
      <div class="c_FormAdd_editor">
        <div class="c_FormAdd_editor_head clearfix" @click="ddjsDataShow = !ddjsDataShow">
          <p>
            督导纪实
            <span class="fa fa-sort-desc fr" v-show="!ddjsDataShow"></span>
            <span class="fa fa-sort-asc fr" v-show="ddjsDataShow"></span>
          </p>
        </div>
        <div class="c_FormAdd_editor_content" v-show="ddjsDataShow">
          <vue-html5-editor :content="ddjsData" :height="200" @change="updateDdjsData"></vue-html5-editor>

          <cell v-show="ddjsDataShow" title="查阅资料(份)">
            <inline-x-number style="display:block;" :min="0" v-model="data.CYZL" width="50px" button-style="round"></inline-x-number>
          </cell>
          <cell v-show="ddjsDataShow" title="列席会议(次)">
            <inline-x-number style="display:block;" :min="0" v-model="data.LXHY" width="50px" button-style="round"></inline-x-number>
          </cell>
          <cell v-show="ddjsDataShow" title="座谈走访(次)">
            <inline-x-number style="display:block;" :min="0" v-model="data.ZTZF" width="50px" button-style="round"></inline-x-number>
          </cell>
          <cell v-show="ddjsDataShow" title="问卷调查(次)">
            <inline-x-number style="display:block;" :min="0" v-model="data.WJDC" width="50px" button-style="round"></inline-x-number>
          </cell>
          <cell v-show="ddjsDataShow" title="校园巡视(次)">
      ...
      <div class="c_FormAdd_editor">
        ...
        <div v-show="experienceAndPractice" class="">
    ...
    <!--工作计划隐藏页-->
    <div v-show="wokrPlanShow && !ddpgShow" class="c_pickBody">
      <div class="c_pickBody_head clearfix">
        <x-button mini class="fl" type="primary" plain @click.native="wokrPlanShow = !wokrPlanShow">
          <span class="fa fa-angle-left fa-1x"></span>
          <span class="text-bold">&nbsp;选择</span>
        </x-button>
        <span class="fr" @click.stop="wokrPlanSearchShow = true">
          <span class="fa fa-search">&nbsp;搜索</span>
        </span>
      </div>
      <div class="" v-show="wokrPlanSearchShow">
        <group title="搜索条件" class="c_searchBody_content">
          <x-input title="学校名称" class="c_inputParent_borderB" v-model="wokrPlanSearch.xxmc" placeholder="请输入学校名称"></x-input>
          <x-button  @click.native.stop="searchWorkPlanData">搜索</x-button>
        </group>
        <div class="c_searchBody_mask" @click.stop="wokrPlanSearchShow = !wokrPlanSearchShow"></div>
      </div>
      <div>
        <checklist :max="1" :min="1" :options="wokrPlanDataList" @on-change="addWorkPlan"></checklist>
      </div>
      <x-button @click.native="loadWorkPlanData(true)" :disabled="wokrPlanMoreParam.hasMore">{{wokrPlanMoreParam.moreName}}</x-button>
    </div>

    <!--学校选择隐藏页-->
    <div  ref="ksText" v-show="xx.xxShow" class="c_pickBody">
      <div class="c_pickBody_head clearfix">
    ...
    <div v-show="sxdxShow && !ddpgShow" class="c_pickBody">
      ...
      <div class="" v-show="sxdxSearchShow">
        <group title="搜索条件" class="c_searchBody_content">
          <x-input title="姓名" class="c_inputParent_borderB" v-model="sxdxSearch.xm" placeholder="请输入随行督学姓名"></x-input>
          ...
          <x-button  @click.native.stop="searchSxdxData">搜索</x-button>
        </group>
    ...
    <!-- 后续处理意见隐藏页-->
    <div v-show="disposeIdeasShow.bodyShow && !ddpgShow" class="c_ddjs_add_info">
      <!--小问题:4-->
...
<script>
import {XHeader, Group, PopupPicker, XTextarea, XButton, Cell, XInput, Checklist, Previewer, Datetime, InlineXNumber, XSwitch, Rater, Confirm} from 'vux'
import { mapGetters } from 'vuex'
export default {
  name: 'ddjs_add',
  components: {XHeader, Group, PopupPicker, XTextarea, XButton, Cell, XInput, Checklist, Previewer, Datetime, InlineXNumber, XSwitch, Rater, Confirm},
  data () {
    return {
      // 随行督学中搜索的隐藏显示
  ...
  methods: {
    ...
    toBack () {
      if (this.ddpgShow) {
        ...
        // 如果是修改(编辑)进来的 且 修改过后续处理意见 改状态为0 并删除整改id
        if (this.contentId && (this.disposeIdeaInit !== this.disposeIdeaValue[0])) {
          let contentid = []
          contentid.push(this.contentId)
          contentid = JSON.stringify(contentid)
          // 修改后续处理意见状态
          this.$kwz.ajax.ajaxUrl({
            url: 'ddjl/changeStatus/DDJL',
            type: 'POST',
            data: {
              CONTENT_IDS: contentid // '{a:'}'
            },
            then (response) {
              if (response.data.statusCode !== '200') {
                console.error('后续处理意见状态未删除成功')
              }
            }
          })
          this.deleteDisposeIdeaId()
        }
        // 从新增来的 不保存直接返回要删除后续处理意见
        ...
        this.$kwz.ajax.ajaxUrl({
          ...
          then (response) {
            ...
            if (datas && datas.CONTENT_ID) {
              this.funName = '查看督导记录'
              this.workPlanId = datas.GZAP_YWID
              ...
              this.workPlanName = workPlanName && workPlanName.length > 12 ? workPlanName.substr(0, 12) : workPlanName
              this.data.xxValue = datas.ORG_ID
              this.data.xxName = datas.XXMC
              this.data.YWSJ = datas.YWSJ
              this.disposeIdeaValue[0] = datas.STATUS
              this.disposeIdeaInit = datas.STATUS // 现存好进来时的状态值，等返回的时候来判断有没有修改
              this.disposeIdea.STATUS = datas.STATUS
              this.disposeIdea.STATUS_MC = this.$refs.hxText.getNameValues()
              this.sxdxName = datas.USERNAME
              this.sxdxId = datas.USERID
              this.data.DXJY = datas.DXJY || ''
              this.experienceAndPractice = !!datas.DXJY
              this.data.CZWT = datas.CZWT || ''
              this.existIssue = !!datas.CZWT
              this.data.CYZL = datas.CYZL || 0
              this.data.LXHY = datas.LXHY || 0
              this.data.ZTZF = datas.ZTZF || 0
              this.data.WJDC = datas.WJDC || 0
              this.data.XYXS = datas.XYXS || 0
    ...
    updateDdpg (isNotShow) {
      ...
      if (this.data.MXID) {
        this.$kwz.ajax.ajaxUrl({
          ...
          then (response) {
            let datas = response.data.datas
            if (datas && datas.tbmbglData && datas.tbmbglData.MBNR) {
              this.$kwz.ajax.ajaxUrl({
                ...
                then (response) {
                  ...
                  if (datas1 && datas1.SJ) {
                    try {
                      ...
                      this.pgbzMc = this.BZMC + '(分值：' + sj.FenZhi + '分)'
                      this.pgFz = Math.round(sj.FenZhi)
                    } catch (e) {
                      console.error(e)
                    }
                  }
    ...
    // 搜索随行督学数据
    searchSxdxData () {
      this.sxdxSearchShow = false
      this.sxdxSearch.page = 1
      this.loadSxdxData()
    },
    // 点击添加随行督学
    addSxdx (value, label) {
//      label.unshift(this.user.name)  将本人加进随行督学
    ...
    // 显示工作计划列表
    showWrokPlan () {
      this.wokrPlanShow = !this.wokrPlanShow
      if (this.wokrPlanDataList.length < 1) {
        this.wokrPlanSearch.page = 1
    ...
    searchWorkPlanData () {
      this.wokrPlanSearch.page = 1
      this.wokrPlanSearchShow = false
      this.loadWorkPlanData()
    },
    // 加载工作计划列表数据
    loadWorkPlanData (type) {
      this.$kwz.ajax.ajaxUrl({
        url: 'dd_gzap/doList/DDGZAP',
        type: 'POST',
        data: {
          page: this.wokrPlanSearch.page,
          SEARCH_TEXT: this.wokrPlanSearch.xxmc
        },
        vue: this,
        then (response) {
          let datas = response.data.datas
          if (datas && datas.length > 0) {
            let wokrPlanDataList = []
            for (let i = 0; i < datas.length; i++) {
              let workPlan = datas[i]
              this.wokrPlanData[workPlan.CONTENT_ID] = workPlan
              wokrPlanDataList.push({
                key: workPlan.CONTENT_ID,
                value: `${workPlan.XXMC}/${workPlan.YWSJ}/${workPlan.SD === '1' ? '上午' : '下午'}/${workPlan.AUTHOR}`
              })
            }
            this.wokrPlanSearch.page++
            this.wokrPlanMoreParam.hasMore = false
            this.wokrPlanMoreParam.moreName = '加载更多'
            if (type) {
              this.wokrPlanDataList = this.wokrPlanDataList.concat(wokrPlanDataList)
            } else {
              this.wokrPlanDataList = wokrPlanDataList
            }
          } else {
            this.wokrPlanMoreParam.hasMore = true
            if (type) {
              this.wokrPlanMoreParam.moreName = '已无更多数据'
            } else {
              this.wokrPlanDataList = []
              this.wokrPlanMoreParam.moreName = '无数据'
            }
          }
        }
      })
    ...
    loadNowWorkPlanData () {
      this.$kwz.ajax.ajaxUrl({
        ...
        then (response) {
          let datas = response.data.datas
          this.data.BZID = datas.map.BZID
        }
      })
    },
    // 点击添加工作计划 addWorkPlan
    addWorkPlan (value) {
      if (value[0]) {
        this.workPlanId = value[0]
        let gzjh = this.wokrPlanData[value[0]]
        if (gzjh) {
          // this.data.pgid
          let mc = (gzjh.XXMC ? (gzjh.XXMC + '/') : gzjh.XXMC) + (gzjh.YWSJ ? (gzjh.YWSJ + '/') : gzjh.YWSJ) + (gzjh.SD === '1' ? '上午' : '下午') + '/' + (gzjh.AUTHOR ? (gzjh.AUTHOR + '/') : gzjh.AUTHOR)
          this.workPlanName = mc.length > 12 ? (mc.substr(0, 12) + '...') : mc
          this.data.xxValue = gzjh.ORG_ID_TARGET
          this.data.xxName = gzjh.XXMC
          this.sxdxId = gzjh.CJID
          this.sxdxName = gzjh.CJID_MC
          this.data.YWSJ = gzjh.YWSJ && gzjh.YWSJ.length > 10 ? gzjh.YWSJ.substr(0, 10) : this.$kwz.formatDate('yyyy-MM-dd')
          ...
          this.ddsxShow = !!this.ddsxText || this.previewerListDDSX.length > 0
          this.xqValue = [gzjh.XQID]
          this.ddsx = gzjh.TXT || '无'
          if (gzjh.BZID) {
            this.BZMC = gzjh.BZMC
